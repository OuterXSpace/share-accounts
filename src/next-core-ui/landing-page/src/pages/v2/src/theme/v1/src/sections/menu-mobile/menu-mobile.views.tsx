...
import { MenuMobileStyle } from './menu-mobile.styles';
...
export const LandingPageMenuMobileV2: React.FC<ILandingPageMenuMobileV2Props> = (props) => {
  ...
  return (
    <>
      ...
      >
        <div
          className={`fixed top-0 left-0 z-[11] w-full h-screen bg-opacity-30 bg-[#050512] ${
            !isToggleMenu && 'hidden'
          }`}
        ...
        >
          <div
            className={`text-right text-[3.6vw] tracking-[0.5vw] h-[5vw] flex flex-col uppercase transition-all ease duration-300  ${
              isToggleMenu ? 'animation-slide-down-menu' : 'animation-slide-top-menu'
            }`}
          >
            <h1>menu</h1>
          </div>
          <div
            className={`text-right text-[3.6vw] tracking-[0.5vw] h-[5vw] flex flex-col uppercase transition-all ease duration-300  ${
              isToggleMenu && 'animation-slide-down-menu'
            }`}
          >
            <h1>close</h1>
          </div>
        ...
        <div
          className={`grid col-span-4-12 translate-x-full translate-y-full bg-white fixed gap-[2vw] p-[8vw] rounded-tl-[5vw] bottom-0 right-0 text-black z-[50] uppercase md:p-[4vw] ${
            isToggleMenu ? 'animation-slide-left-menu' : 'animation-slide-right-menu'
          } transition-all ease duration-300`}
        >
          <Link
            href="/"
            aria-current="page"
            className="z-10 no-underline pb-1 text-[6em] md:text-[3em] counter-link before:text-[#A2254B] before:rounded-[50vw] before:px-[0.8vw] before:pt-[0.2vw] before:text-[7px] before:border-[0.5px] before:border-[#A2254B] flex flex-col items-start"
          >
            home
          </Link>
          <Link
